export function PrivacyNotice() {
  return (
    <div className="mt-auto pt-8 pb-6 border-t border-slate-200 text-center bg-lavender">
      <h3 className="text-xs font-bold text-slate-muted uppercase tracking-wider mb-3">
        Privacy Notice
      </h3>
      <p className="text-xs text-slate-muted mx-auto px-4 max-w-4xl sm:whitespace-nowrap">
        Data is local to this device. No personal health data is stored on our servers. Your plan is yours alone.
      </p>
    </div>
  );
}
